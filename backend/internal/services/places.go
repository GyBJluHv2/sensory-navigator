...
func (s *PlaceService) List(filter PlaceFilter) ([]models.Place, error) {
	...
	// Внутренняя структура для сканирования JOIN-результата.
	// Поля Place со всеми колонками таблицы places + агрегаты, которые
	// в самой Place помечены gorm:"-" и потому не попадают в Scan напрямую.
